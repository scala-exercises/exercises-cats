 ...
 * password and submit. Response comes back saying your username can't have dashes in it, so you
 * make some changes and resubmit. Can't have special characters either. Change, resubmit. Passwords
 * need to have at least one capital letter. Change, resubmit. Password needs to have at least one
 * number.
 *
 * Or perhaps you're reading from a configuration file. One could imagine the configuration library
 * you're using returns a `scala.util.Try`, or maybe a `scala.util.Either`. Your parsing may look
 * something like:
 *
 * {{{
 ...
 * }}}
 *
 * You run your program and it says key "url" not found, turns out the key was "endpoint". So you
 * change your code and re-run. Now it says the "port" key was not a well-formed integer.
 *
 * It would be nice to have all of these errors reported simultaneously. That the username can't
 * have dashes can be validated separately from it not having special characters, as well as from
 * the password needing to have certain requirements. A misspelled (or missing) field in a config
 ...
 * =Parallel validation=
 *
 * Our goal is to report any and all errors across independent bits of data. For instance, when we
 * ask for several pieces of configuration, each configuration field can be validated separately
 * from one another. How then do we enforce that the data we are working with is independent? We ask
 * for both of them up front.
 *
 ...
 * }}}
 *
 * Then we enumerate our errors—when asking for a config value, one of two things can go wrong: the
 * field is missing, or it is not well-formed with regards to the expected type.
 *
 * {{{
 ...
 * }}}
 *
 * We need a data type that can represent either a successful value (a parsed configuration), or an
 * error. It would look like the following, which cats provides in `cats.data.Validated`:
 *
 * {{{
 ...
 * validation if each piece is independent. How do we enforce the data is independent? By asking for
 * all of it up front. Let's start with two pieces of data.
 *
 * {{{
 ...
 * }}}
 *
 * Perfect! But, going back to our example, we don't have a way to combine `ConfigError`s. But as
 * clients, we can change our `Validated` values where the error can be combined, say, a
 * `List[ConfigError]`. It is more common however to use a `NonEmptyList[ConfigError]`—the
 * `NonEmptyList` statically guarantees we have at least one value, which aligns with the fact that
 * if we have an `Invalid`, then we most certainly have at least one error. This technique is so
 * common there is a convenient method on `Validated` called `toValidatedNel` that turns any
 * `Validated[E, A]` value to a `Validated[NonEmptyList[E], A]`. Additionally, the type alias
...
object ValidatedSection
  ...
  /**
   * When no errors are present in the configuration, we get a `ConnectionParams` wrapped in a
   * `Valid` instance.
   */
   ...
   * }}}
   *
   * Which can be defined in terms of `Apply#ap` and `Apply#map`, the very functions needed to
   * create an `Apply` instance.
   *
   * Can we perhaps define an `Apply` instance for `Validated`? Better yet, can we define an
   ...
   * }}}
   *
   * Awesome! And now we also get access to all the goodness of `Applicative`, which includes
   * `map{2-22}`, as well as the `Cartesian` syntax `|@|`.
   *
   * We can now easily ask for several bits of configuration and get any and all errors returned
   * back.
   *
   * {{{
   * import cats.Apply
   ...
   *
   * `Option` has `flatMap`, `Either` has `flatMap`, where's `Validated`'s? Let's try to implement
   * it—better yet, let's implement the `Monad` type class.
   *
   * {{{
   * import cats.Monad
   *
   ...
   * of `flatMap` would be inconsistent with that of `ap`, not good. Therefore, `Validated` has only
   ...
   * ==`withEither`==
   *
   * The `withEither` method allows you to temporarily turn a `Validated` instance into an `Either`
