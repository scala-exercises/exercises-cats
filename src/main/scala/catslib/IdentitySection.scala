 ...
 * `A` as values of type `Id[A]`, and vice-versa.
 *
 * {{{
 * import cats._
 *
...
object IdentitySection
  ...
  /**
   * Using this type declaration, we can treat our Id type constructor as a `Monad` and as a
   * `Comonad`. The `pure` method, which has type `A => Id[A]` just becomes the identity function.
   ...
   * }}}
   *
   * You'll notice that in the flatMap signature, since `Id[B]` is the same as `B` for all B, we can
   * rewrite the type of the `f` parameter to be `A => B` instead of `A => Id[B]`, and this makes
   * the signatures of the two functions the same, and, in fact, they can have the same
   * implementation, meaning that for `Id`, `flatMap` is also just function application:
   *
   * {{{
   * import cats.Monad
   *
